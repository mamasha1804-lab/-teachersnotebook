...
function render(){const app=$('#app');if(!account){renderAuth(app);return}const studentIcon='<img src="'+ETP_ASSETS['students-pencil.svg']+'" alt="">';const paidIcon='<img src="'+ETP_ASSETS['paid-coin-custom.webp']+'" alt="Монетка">';app.innerHTML=`<div class="app-shell"><aside class="sidebar"><div class="brand"><span>TEACHER <img class="brand-heart" src="${ETP_ASSETS['heart.svg']}" alt=""></span><span>DAY</span></div><div class="nav">${navButton('today','🏠','Сегодня')}${navButton('calendar','🗓️','Календарь')}${navButton('students',studentIcon,'Списки учеников по классам')}${navButton('schedule','📖','Моё расписание')}${navButton('payments',paidIcon,'Платные занятия')}${navButton('homework','🏫','Домашние задания')}${navButton('materials','🗂️','Материалы')}</div><button class="logout" data-logout>Выйти · ${esc(account.login)}</button></aside><main class="main">${hero()}${view==='today'?todayView():weekView()}</main></div>${decorations()}${modal?renderModal():''}`;bind();updateClock()}
